...
var WebURL         = "https://eth.pool2mine.net/";
...
function loadGeneralData() {
	return $.ajax(API + "pools").done(function(data) {
		var coinLogo = "";
		var geckoLogo = "";
		var poolHeader = "";
		var poolHeader2 = "";
		...
		$.each(data.pools, function(index, value) {
			if (currentPool === value.id) {
				...
				coinWebLink += "<a href='" + coinWeb + "' target='_blank'><i class='bx bx-world'></i> " + value.coin.name + " Website</a>";
				coinGitLink += "<a href='" + coinGit + "' target='_blank'><i class='bx bxl-github'></i> " + value.coin.name + " Github</a>";
	...
	return $.ajax(API + "pools").done(function(data) {
		$.each(data.pools, function(index, value) {
			if (currentPool === value.id) {
				...
				$("#minimumPaymentTicker").text(value.paymentProcessing.minimumPayment + " " + value.coin.type);
				$("#payoutSchemeTicker").text(value.paymentProcessing.payoutScheme);
				$("#poolFeePercentTicker").text(value.poolFeePercent + " %");
				$("#poolHashrateTicker").text(_formatter(value.poolStats.poolHashrate, 5, "H/s"));
				$("#poolMinersTicker").text(value.poolStats.connectedMiners + " Miner(s)");
				$("#poolWorkersTicker").text(value.poolStats.connectedWorkers + " Worker(s)");
				$("#networkHashrateTicker").text(_formatter(value.networkStats.networkHashrate, 3, "H/s"));
				$("#networkDifficultyTicker").text(_formatter(value.networkStats.networkDifficulty, 3, "H/s"));
				$("#poolPaymentIntervalTicker").text(readableSeconds(value.paymentProcessing.extra.paymentInterval));
			}
		});
	})
	.fail(function() {
...
function loadStatsData() {
	return $.ajax(API + "pools").done(function(data) {
		$.each(data.pools, function(index, value) {
			if (currentPool === value.id) {
				var ttf = Math.round(value.networkStats.networkHashrate / value.poolStats.poolHashrate * value.blockTimeInterval);
				var effort = ((value.poolStats.sharesDiff / value.networkStats.networkDifficulty) * 100).toFixed(2);
				var lastPoolBlock = convertUTCDateToLocalDate(new Date(value.lastPoolBlockTime),false);
				...
				$("#netHashrate").text(_formatter(value.networkStats.networkHashrate, 3, "H/s"));
				$("#networkDifficulty").text(_formatter(value.networkStats.networkDifficulty, 3, "H/s"));
				$("#netDifficulty").text(_formatter(value.networkStats.networkDifficulty, 3, "H/s"));
				$("#poolTTF").text(readableSeconds(ttf));
				$("#poolEffort").text(effort + " %");
				$("#poolPaymentInterval").text(readableSeconds(value.paymentProcessing.extra.paymentInterval));
				$("#currentShares").text(_formatter(value.poolStats.sharesDiff, 5, ""));
...
function loadStatsChart() {
	return $.ajax(API + "pools/" + currentPool + "/performance").done(function(data) {
		labels = [];	  
		poolHashrate = [];
		networkHashrate = [];
		networkDifficulty = [];
		...
		var options		= {height: "377px",showArea: true,showPoint: false,seriesBarDistance: 1,axisX: {showGrid: false},
		...
		var chartNetdiff	= {height: "125px",showArea: true,showPoint: false,seriesBarDistance: 1,axisX: {showGrid: false},
					  ...
					  axisY: {offset: 47,scale: "logcc",labelInterpolationFnc: function(value) {return _formatter(value, 1, "H/s");}},
					  lineSmooth: Chartist.Interpolation.simple({divisor: 2})};
		var chartMiners		= {height: "125px",showArea: true,showPoint: false,seriesBarDistance: 1,axisX: {showGrid: false},
					  fullWidth: true, chartPadding: { right: 10, left: -5, bottom: -10},
					  axisY: {offset: 47,scale: "logcc",labelInterpolationFnc: function(value) {return _formatter(value, 1, "");}},
					  lineSmooth: Chartist.Interpolation.simple({divisor: 2})};
		var chartWorkers	= {height: "125px",showArea: true,showPoint: false,seriesBarDistance: 1,axisX: {showGrid: false},
					  fullWidth: true, chartPadding: { right: 10, left: -5, bottom: -10},
					  axisY: {offset: 47,scale: "logcc",labelInterpolationFnc: function(value) {return _formatter(value, 1, "");}},
					  lineSmooth: Chartist.Interpolation.simple({divisor: 2})};
		var responsiveOptions	= [["screen and (max-width: 640px)",{axisX: {labelInterpolationFnc: function(value) {return value[1];}}}]];
		Chartist.Line("#chartStatsHashrate", dataNetworkHash, chartNethash, responsiveOptions);
		Chartist.Line("#chartStatsHashratePool", dataPoolHash, options, responsiveOptions);
		Chartist.Line("#chartStatsDiff", dataNetworkDiff, chartNetdiff, responsiveOptions);
		Chartist.Line("#chartStatsMiners", dataMiners, chartMiners, responsiveOptions);
		Chartist.Line("#chartStatsWorkers", dataWorkers, chartWorkers, responsiveOptions);
	})
	.fail(function() {
		$.notify(
...
function loadStatsPrice() {
	return $.ajax(geckoAPI + geckoCOIN).done(function (data) {
		...
		$("#coinToUSD").html("$ " + data.market_data.current_price.usd.toFixed(2));
...
function loadDashboardData(walletAddress) {
	...
	$.ajax(API + "pools/" + currentPool + "/miners/" + walletAddress).done(function(data) {
		...
		$("#pendingBalance").text(_formatter(data.pendingBalance, 8, " " + coinTicker));
		$("#paidBalance").text(_formatter(data.todayPaid, 5, ""));
		$("#lifetimeBalance").text(_formatter(data.pendingBalance + data.totalPaid, 5, " " + coinTicker));
...
function loadDashboardWorkerList(walletAddress) {
	return $.ajax(API + "pools/" + currentPool + "/miners/" + walletAddress).done(function(data) {
		var workerList = "";
		if (data.performance) {
			var workerCount = 0;
			$.each(data.performance.workers, function(index, value) {
				...
				workerList += "<td class='text-white'>" + _formatter(value.sharesPerSecond, 3, "S/s") + "</td>";
				workerList += "</tr>";
			});
		} else {
...
function loadDashboardAvgHash(walletAddress) {
	return $.ajax(API + "pools/" + currentPool + "/miners/" + walletAddress).done(function(data) {
		if (data.performanceSamples.length > 11) {
			var ht1 = 0; var ht2 = 0; var ht3 = 0; var ht4 = 0; var ht5 = 0; var ht6 = 0;
			var ht7 = 0; var ht8 = 0; var ht9 = 0; var ht10 = 0; var ht11 = 0; var ht12 = 0;
			$.each(data.performanceSamples[0].workers, function(index, value) {ht1 += value.hashrate;});
			$.each(data.performanceSamples[1].workers, function(index2, value2) {ht2 += value2.hashrate;});
			$.each(data.performanceSamples[2].workers, function(index3, value3) {ht3 += value3.hashrate;});
			$.each(data.performanceSamples[3].workers, function(index4, value4) {ht4 += value4.hashrate;});
			$.each(data.performanceSamples[4].workers, function(index5, value5) {ht5 += value5.hashrate;});
			$.each(data.performanceSamples[5].workers, function(index6, value6) {ht6 += value6.hashrate;});
			$.each(data.performanceSamples[6].workers, function(index7, value7) {ht7 += value7.hashrate;});
			$.each(data.performanceSamples[7].workers, function(index8, value8) {ht8 += value8.hashrate;});
			$.each(data.performanceSamples[8].workers, function(index9, value9) {ht9 += value9.hashrate;});
			$.each(data.performanceSamples[9].workers, function(index10, value10) {ht10 += value10.hashrate;});
			$.each(data.performanceSamples[10].workers, function(index11, value11) {ht11 += value11.hashrate;});
			$.each(data.performanceSamples[11].workers, function(index12, value12) {ht12 += value12.hashrate;});
			var avgtothash = ((ht1 + ht2 + ht3 + ht4 + ht5 + ht6 + ht7 + ht8 + ht9 + ht10 + ht11 + ht12) / 12);
...
function loadDashboardChart(walletAddress) {
	return $.ajax(API + "pools/" + currentPool + "/miners/" + walletAddress + "/performance").done(function(data) {
		labels = [];
		minerHashRate = [];	
		...
		var options		= {height: "250px",showArea: true,showPoint: false,seriesBarDistance: 1,axisX: {showGrid: false},
					  fullWidth: true, chartPadding: { right: 10, left: -5, bottom: -10},
...
function loadMinersPage() {
	...
	$.ajax(API + "pools/" + currentPool + "/miners?page=0&pagesize=100").done(function(data) {
		var minerList = "";
		if (data.length > 0) {
			var minerCount = 0;
			$.each(data, function(index, value) {
				...
				minerList += "<td class='text-white'><a href='dashboard.html?#" + currentPool + "/stats?address=" + value.miner + "'>" + value.miner + "</a></td>";
				minerList += "<td class='text-white'>" + _formatter(value.sharesPerSecond, 5, "S/s") + "</td>";
				minerList += "<td class='text-white'>" + _formatter(value.hashrate, 5, "H/s (Average Last Hour)") + "</td>";
...
function loadBlocksPage() {
	return $.ajax(API + "pools/" + currentPool + "/blocks?page=0&pageSize=1000").done(function(data) {
		var blockList = "";
		if (data.length > 0) {
			$.each(data, function(index, value) {
				var createDate = convertUTCDateToLocalDate(new Date(value.created),false);
				...
				blockList += "<td class='text-white'><a href='dashboard.html?#" + currentPool + "/stats?address=" + value.miner + "'>" + value.miner.substring(0, 8) + " &hellip; " + value.miner.substring(value.miner.length - 8) + "</a></td>";
				blockList += "<td><a href='" + value.infoLink + "' target='_blank'>" + value.blockHeight + "</a></td>";
				blockList += "<td class='text-white'>" + _formatter(value.networkDifficulty, 5, "H/s") + "</td>";
				...
				if (typeof value.status !== "confirmed") {
					...
					blockList += "<td><span class='badge badge-primary'>Pending</span></td>";
				} else if (typeof value.status !== "orphaned") {
					blockList += "<td><span class='badge badge-danger'>Orphaned</span></td>";
				} else {
					blockList += "<td>" + status + "</td>";
				}
				blockList += "<td class='text-white'>" + _formatter(value.reward, 5, "") + "</td>";
				blockList += "<td><div class='progress-bar progress-bar-striped bg-info progress-bar-animated' role='progressbar' aria-valuenow='" + calcs + "' aria-valuemin='0' aria-valuemax='100' style='width: " + calcs + "%'><span class='text-white'>" + calcs + "% Completed</span></div></td>";
				blockList += "</tr>";
			});
		} else {
...
function loadAvgLuck16Page() {
	return $.ajax(API + "pools/" + currentPool + "/blocks?page=0&pageSize=16").done(function (data) {
		var luckList16 = "";
		if (data.length > 0) {
			var total = 0;
			$.each(data, function (index, value) {
...
function loadAvgTotalPage() {
	return $.ajax(API + "pools/" + currentPool + "/blocks?page=0&pageSize=100000").done(function (data) {
		var luckListAll = "";
		if (data.length > 0) {
			var total = 0;
			var totalCoin = 0;
...
function loadLastBlock() {
	return $.ajax(API + "pools").done(function(data) {
		$.each(data.pools, function(index, value) {
			if (currentPool === value.id) {
				var lastPoolBlocks = convertUTCDateToLocalDate(new Date(value.lastPoolBlockTime),false);
...
function loadPaymentsPage() {
	return $.ajax(API + "pools/" + currentPool + "/payments?page=0&pageSize=1000000").done(function(data) {
		var paymentList = "";
		if (data.length > 0) {
			$.each(data, function(index, value) {
				var createDate = convertUTCDateToLocalDate(new Date(value.created),false);
				...
				paymentList += '<td class="text-success">' + _formatter(value.amount, 5, ' ETH') + '</td>';
	...
	return $.ajax(API + "pools").done(function(data) {
		...
		$.each(data.pools, function(index, value) {
			if (currentPool === value.id) {
				...
				connectPoolConfig += "<tr><td class='text-primary'>Crypto Coin Name</td><td class='text-white'>" + coinName + " (" + value.coin.type + ") </td></tr>";
				connectPoolConfig += "<tr><td class='text-primary'>Coin Algorithm</td><td class='text-white'>" + value.coin.algorithm + "</td></tr>";
				connectPoolConfig += "<tr><td class='text-primary'>Coin Reward Type</td><td class='text-white'>" + value.networkStats.rewardType + "</td></tr>";
				connectPoolConfig += '<tr><td class="text-primary">Pool Wallet</td><td><a href="' + value.addressInfoLink + '" target="_blank">' + value.address.substring(0, 12) + " &hellip; " + value.address.substring(value.address.length - 12) + "</a></td></tr>";
				connectPoolConfig += "<tr><td class='text-primary'>Payout Scheme</td><td class='text-white'>" + value.paymentProcessing.payoutScheme + "</td></tr>";
				connectPoolConfig += "<tr><td class='text-primary'>Minimum Payment</td><td class='text-white'>" + value.paymentProcessing.minimumPayment + " " + value.coin.type + "</td></tr>";
				connectPoolConfig += "<tr><td class='text-primary'>Pool Fee</td><td class='text-white'>" + value.poolFeePercent + "%</td></tr>";
        		}
